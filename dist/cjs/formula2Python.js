...
const formula2Python = function(formula, options) {
        ...
        switch (token.subtype) {
          case constants.TOK_SUBTYPE_RANGE:
            ...
            switch (currentFunctionOnStack.name.toLowerCase()) {
              ...
              case "and":
                if (RegExp(":", "gi").test(tokenString)) {
                  outStr = breakOutRanges.breakOutRanges(tokenString, " and ");
                } else {
                  outStr = tokenString;
                }
                break;
              case "or":
                if (RegExp(":", "gi").test(tokenString)) {
                  outStr = breakOutRanges.breakOutRanges(tokenString, " or ");
                } else {
                  outStr = tokenString;
                }
                break;
  ...
  const pythonOutput = formatFormula.formatFormula(formula, options);
  return pythonOutput;
};
exports.formula2Python = formula2Python;
