...
const getTokens = require("./getTokens.js");
...
const formatFormula = function(formula, options) {
  ...
  const tokens = getTokens.getTokens(formula, options.isEu);
  let outputFormula = "";
  ...
  const testAutoBreak = function(nextToken) {
    ...
    for (; i < autoBreakArray.length; i += 1) {
      if (nextToken !== null && typeof nextToken !== "undefined" && (constants.types[autoBreakArray[i]] === nextToken.type.toString() || constants.types[autoBreakArray[i]] === nextToken.subtype.toString())) {
        return true;
      }
    }
    return false;
  ...
  while (tokens.moveNext()) {
    ...
    if (token.subtype.toString() === constants.TOK_SUBTYPE_STOP) {
    ...
    if (token.subtype.toString() === constants.TOK_SUBTYPE_START) {
      indentCount += 1;
    }
